...
class CustomSave {
    ...
    public function get_result_set($params, $aggregated=false){
      global $wpdb;
      ...
      $dbresult = $wpdb->select($wpdb->prepare( 
       "SELECT 
          d.email as email, 
          d.plan as plan, 
        ...
        AND c.created <= %d
        AND c.email = d.email
        ORDER BY %s %s", $options['start'], $options['end'], $options['orderby'], $options['order']), ARRAY_A);
...
// still here? you must want the compiled aggregated results
      ...
      foreach ($dbresult as $key => $value) {
          if(!isset($results[$value['email']])){
            ...
            $results[$value['email']]['payments'] = 1;
          } else {
            $results[$value['email']]['payments'] += 1;
